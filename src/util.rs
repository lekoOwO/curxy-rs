...
pub fn convert_to_custom_endpoint(url: &str, endpoint: &str) -> Result<String> {
    ...
    new_url.set_scheme(endpoint_url.scheme()).map_err(|_| anyhow::anyhow!("Failed to set scheme"))?;
    new_url.set_host(Some(endpoint_url.host_str().ok_or_else(|| anyhow::anyhow!("No host in endpoint"))?)).map_err(|_| anyhow::anyhow!("Failed to set host"))?;
    if let Some(port) = endpoint_url.port() {
        new_url.set_port(Some(port)).map_err(|_| anyhow::anyhow!("Failed to set port"))?;
    }
...
mod tests {
    ...
    fn test_choose_endpoint() {
        ...
        assert_eq!(
            choose_endpoint("gpt-3.5", ollama, openai),
            openai
        );
        assert_eq!(
            choose_endpoint("llama2", ollama, openai),
            ollama
        );
    }
